...
```{r linespm, echo=FALSE}
# How many lines of text per meeting?
gcfboard_docs %>%
  group_by(meeting) %>%
  summarise(lines = n_distinct(text)) %>% 
  ggplot(aes(meeting, lines)) +
  geom_bar(stat = "identity") +
  labs(title = "Lines of text produced per board meeting")
```
...
```{r changes, fig.width=9, fig.height=12}
# Usage over time for selected words
changes <- filter(words_by_meeting, word %in% c("secretariat", "project", "risk", "private")) %>%
  filter(meeting != "B.04") %>% 
  ggplot(aes(meeting, count/meeting_total, color = word)) +
  geom_point(size = 1.3) +
  geom_line(aes(group = word)) +
  labs(x = NULL, y = "Word frequency (count/meeting_total)") 

c2 <- changes + scale_y_log10() + labs(x = NULL, y = "log10(Word frequency)") 
c3 <- changes + coord_trans(y = "sqrt") + labs(x = NULL, y = "sqrt(Word frequency)") 
...
plot_bm %>%
  group_by(meeting) %>%
  filter(word != "page" & word != "anxjointmgten.doc" & word != "ÿ" & meeting == "B.16") %>% 
  top_n(8) %>%
  ungroup %>%
  arrange(desc(tf_idf)) %>%
  ggplot(aes(word, tf_idf, fill = word)) +
  geom_col(show.legend = FALSE) +
  labs(title = "Words particularly associated with B.16", x = NULL, y = "Term Frequency - Inverse Document Frequency") +
  theme(axis.text.x = element_blank()) +
  theme(axis.ticks.x = element_blank()) +
  coord_flip()
```
...
plot_bm %>%
  group_by(meeting) %>%
  filter(word != "page" & word != "anxjointmgten.doc" & word != "ÿ" & meeting == "B.06") %>% 
  top_n(8) %>%
  ungroup %>%
  arrange(desc(tf_idf)) %>% 
  ggplot(aes(word, tf_idf, fill = tf_idf)) +
  geom_col(show.legend = FALSE) +
  labs(title = "Words particularly associated with B.06", x = NULL, y = "Term Frequency - Inverse Document Frequency") +
  theme(axis.text.x = element_blank()) +
  theme(axis.ticks.x = element_blank()) +
  scale_fill_gradient2() +
  coord_flip()
